import os
import warnings
from contextlib import nullcontext
from typing import Tuple

import torch
from transformers import AutoTokenizer, AutoModelForSequenceClassification, logging as hf_logging

MODEL_DIR  = "./earnings-aiera-finbert"
MAX_LENGTH = int(os.getenv("COMPANY_SENT_MAXLEN", "512"))
USE_FP16   = True

_tok = None
_model = None
_device = None
_model_type = None
_max_length = MAX_LENGTH

def _lazy_load():
    global _tok, _model, _device, _model_type, _max_length
    if _tok is not None:
        return
    os.environ.setdefault("TRANSFORMERS_VERBOSITY", "error")
    os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")
    warnings.filterwarnings("ignore", category=FutureWarning)
    warnings.filterwarnings("ignore", category=UserWarning)
    hf_logging.set_verbosity_error()

    _device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    _tok = AutoTokenizer.from_pretrained(MODEL_DIR, use_fast=True)
    _model = AutoModelForSequenceClassification.from_pretrained(MODEL_DIR).to(_device).eval()
    _model_type = getattr(_model.config, "model_type", None)
    model_cap = getattr(_model.config, "max_position_embeddings", None)
    tok_cap = getattr(_tok, "model_max_length", None)
    caps = [MAX_LENGTH]
    if isinstance(model_cap, int) and model_cap > 0:
        caps.append(model_cap)
    if isinstance(tok_cap, int) and 0 < tok_cap < 100000:
        caps.append(tok_cap)
    _max_length = int(min(caps))

def predict(text: str) -> Tuple[str, float]:
    _lazy_load()
    if not text or not text.strip():
        return ("", 0.0)

    enc = _tok(text, return_tensors="pt", truncation=True, max_length=_max_length)
    input_ids = enc["input_ids"].to(_device)
    attention_mask = enc["attention_mask"].to(_device)

    extra = {}
    if _model_type == "longformer":
        global_attention_mask = torch.zeros_like(input_ids)
        global_attention_mask[:, 0] = 1
        extra["global_attention_mask"] = global_attention_mask

    amp_ctx = (
        torch.amp.autocast(device_type="cuda", dtype=torch.float16)
        if (USE_FP16 and _device.type == "cuda")
        else nullcontext()
    )
    with torch.inference_mode(), amp_ctx:
        out = _model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            **extra,
        )
        probs = torch.softmax(out.logits, dim=-1)
        pred_id = int(probs.argmax(dim=-1).item())
        conf = float(probs[0, pred_id].item())

    label = _model.config.id2label.get(pred_id, str(pred_id))
    label = label.capitalize()
    return (label, conf)
