...
def is_valid_example(ex: Dict[str, Any]) -> bool:
    ...
    return isinstance(t, str) and isinstance(s, str) and t.strip() != "" and s.strip() != ""
...
def main():
    ...
    full = {"train": split_train["train"], "validation": split_train["test"], "test": ds_clean["test"]}
    if args.quick_test:
    ...
    tokenized = {split: d.map(preprocess, batched=True, remove_columns=remove_cols) for split, d in full.items()}
    ...
    if len(full["test"]) > 0:
        ...
        inputs = tokenizer([f"summarize: {raw_doc}"], max_length=args.max_input_length, truncation=True, return_tensors="pt")
    ...
    if per_epoch:
        ...
        plt.close()
    try:
        n_samples = min(64, len(full["test"]))
        if n_samples > 0:
            preds_out = trainer.predict(tokenized["test"].select(range(n_samples)))
            pred_ids = preds_out.predictions
            preds = tokenizer.batch_decode(pred_ids, skip_special_tokens=True)
            refs = full["test"].select(range(n_samples))["Summary"]
            inputs_raw = full["test"].select(range(n_samples))["text"]
    ...
    run_settings = {
        ...
        "dataset_sizes": {"train": len(full["train"]), "validation": len(full["validation"]), "test": len(full["test"])},
