...
MODEL_DIR = Path(os.getenv("CB_CUSTOM_MODEL_DIR", "runs/cb_hier_rnn/cb_hier_rnn_v2"))
...
def _load_latest_model() -> bool:
    ...
    try:
        ...
        spm_path = None
        for tok_name in ("tokenizer.model", "spm.model"):
            candidate = MODEL_DIR / tok_name
            if candidate.exists():
                spm_path = candidate
                break
        if not (ckpt_path.exists() and spm_path is not None):
            _HAS_MODEL = False
            return False
