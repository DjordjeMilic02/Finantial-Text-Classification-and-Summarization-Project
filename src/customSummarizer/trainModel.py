...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history.append({"epoch": epoch, "train_loss": float(tr_loss), "val_loss": float(val_loss), "secs": float(dt)})
    ...
    total_time_s = time.time() - t_train0
    ...
    if history:
        with (out_dir / "metrics_per_epoch.csv").open("w", encoding="utf-8", newline="") as f:
            w = csv.DictWriter(f, fieldnames=["epoch", "train_loss", "val_loss", "secs"])
            w.writeheader()
            for r in history:
                w.writerow(r)
        epochs = [r["epoch"] for r in history]
        tr_vals = [r["train_loss"] for r in history]
        va_vals = [r["val_loss"] for r in history]
        plt.figure(figsize=(7, 4.5))
        plt.plot(epochs, tr_vals, marker="o", label="Train Loss")
        plt.plot(epochs, va_vals, marker="o", label="Val Loss")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
    ...
    model.eval()
    n_eval = min(200, len(test_recs)) if len(test_recs) > 0 else 0
    test_subset = test_recs[:n_eval]
    preds, refs = [], []
    if n_eval > 0:
        test_dl = torch.utils.data.DataLoader(
            HSumDataset(test_subset, sp_tok),
        ...
        with torch.inference_mode():
            for batch in tqdm(test_dl, total=len(test_dl), desc="gen(test)", leave=False):
                for k in batch:
                    batch[k] = batch[k].to(device, non_blocking=True)
                gen_ids = model.generate(batch["enc"], batch["enc_token_mask"], batch["sent_mask"], sp_tok, max_len=MAX_TGT)
                for row in gen_ids.cpu().tolist():
                    preds.append(sp_tok.decode(row).strip())
        refs = [ex["summary"].strip() for ex in test_subset]
        with (out_dir / "predictions_head.csv").open("w", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
            w.writerow(["idx", "input", "reference", "prediction"])
            for i, ex in enumerate(test_subset[:64]):
                w.writerow([i, ex["text"], ex["summary"], preds[i] if i < len(preds) else ""])
    device_str = "cuda:" + str(torch.cuda.current_device()) if torch.cuda.is_available() else "cpu"
    gpu_name = (torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu")
    run_settings = {
        "model_type": "HierRNN-Abstractive",
        "config": {
            "EMB_DIM": EMB_DIM, "WORD_HID": WORD_HID, "SENT_HID": SENT_HID,
            "DEC_HID": DEC_HID, "DROPOUT": DROPOUT,
            "MAX_SENTS": MAX_SENTS, "MAX_WORDS": MAX_WORDS, "MAX_TGT": MAX_TGT,
        ...
        "training": {
            ...
            "warmup_fraction": WARMUP_FRACT,
            "accum_steps": ACCUM_STEPS,
            "amp": AMP
        },
        "dataset_sizes": {"train": len(train_recs), "validation": len(val_recs), "test": len(test_recs)},
        "device": device_str,
        "gpu_name": gpu_name,
