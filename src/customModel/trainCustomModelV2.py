...
class HAN(nn.Module):
    def __init__(self, vocab_size, num_classes):
        ...
        self.word_rnn = nn.GRU(input_size=EMB_DIM, hidden_size=WORD_HID, num_layers=1, batch_first=True, bidirectional=True)
        self.word_attn = WordAttn(in_dim=2*WORD_HID, attn_dim=WORD_HID)
        self.sent_rnn = nn.GRU(input_size=2*WORD_HID, hidden_size=SENT_HID, num_layers=1, batch_first=True, bidirectional=True)
...
def main():
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        history_rows.append({"epoch": epoch, "train_loss": float(tr_loss), "val_loss": float(va_loss), "val_acc": float(va_acc), "val_f1": float(va_f1), "secs": float(dt)})
        if va_f1 > best_f1:
    ...
    if history_rows:
        ...
        with csv_path.open("w", encoding="utf-8", newline="") as f:
            w = csv.DictWriter(f, fieldnames=["epoch","train_loss","val_loss","val_acc","val_f1","secs"])
            w.writeheader()
            for r in history_rows: w.writerow(r)
        epochs = [r["epoch"] for r in history_rows]
        accs = [r["val_acc"] for r in history_rows]
        f1s  = [r["val_f1"] for r in history_rows]
        plt.figure(figsize=(7,4.5))
        if any(v is not None for v in accs):
            plt.plot(epochs, accs, marker="o", label="Val Accuracy")
        if any(v is not None for v in f1s):
            plt.plot(epochs, f1s, marker="o", label="Val Macro-F1")
        plt.xlabel("Epoch"); plt.ylabel("Score"); plt.title("Eval Metrics per Epoch"); plt.grid(True, alpha=0.3); plt.legend(); plt.tight_layout()
        plt.savefig(OUT_DIR / "metrics_per_epoch.png", dpi=150); plt.close()
